...
var eventsForWeek$1 = function eventsForWeek(evts, start, end, accessors) {
  return evts.filter(function (e) {
    return inRange(e, start, end, accessors);
  });
};
...
/*#__PURE__*/
function (_React$Component) {
  _inheritsLoose(WorkMonthView, _React$Component);

  function WorkMonthView() {
    var _this;
    ...
    _this.renderWeek = function (week, weekIdx) {
      ...
      var _this$state = _this.state,
          needLimitMeasure = _this$state.needLimitMeasure,
          rowLimit = _this$state.rowLimit;
      events = eventsForWeek$1(events, week[0], week[week.length - 1], accessors);
      events.sort(function (a, b) {
        return sortEvents(a, b, accessors);
      });
      return React.createElement(DateContentRow, {
  ...
  var _proto = WorkMonthView.prototype;
  ...
  return WorkMonthView;
}(React.Component);

WorkMonthView.propTypes = process.env.NODE_ENV !== "production" ? {
  events: PropTypes.array.isRequired,
  date: PropTypes.instanceOf(Date),
  min: PropTypes.instanceOf(Date),
  max: PropTypes.instanceOf(Date),
...
WorkMonthView.range = function (date, _ref4) {
  var localizer = _ref4.localizer;
  var start = firstVisibleDay(date, localizer);
  var end = lastVisibleDay(date, localizer);
  return {
...
WorkMonthView.navigate = function (date, action) {
  switch (action) {
    case navigate.PREVIOUS:
      return add(date, -1, 'month');
...
WorkMonthView.title = function (date, _ref5) {
  var localizer = _ref5.localizer;
  return localizer.format(date, 'monthHeaderFormat');
};

var _VIEWS;
var VIEWS = (_VIEWS = {}, _VIEWS[views.MONTH] = MonthView, _VIEWS[views.WEEK] = Week, _VIEWS[views.WORK_WEEK] = WorkWeek, _VIEWS[views.DAY] = Day, _VIEWS[views.AGENDA] = Agenda, _VIEWS[views.WORK_MONTH] = WorkMonthView, _VIEWS);
...
function (_React$Component) {
   ...
   * @param date
   * @param viewComponent
   * @param {'month'|'week'|'work_week'|'day'|'agenda|work_month'} [view] - optional
